...
from ..core.dependencies import get_recommendation_service, get_roadmap_service
...
from ..services.roadmap_service import GenerationError, RecommendationService, RoadmapNotFoundError, RoadmapService

router = APIRouter(prefix="/v1/roadmap", tags=["roadmap"])
logger = logging.getLogger(__name__)


@router.post("/generate", response_model=list[Module])
...
@router.get("/user/{user_id}/recommendations", response_model=RecommendationResponse)
def get_recommendations_for_user(
    user_id: str,
    service: RecommendationService = Depends(get_recommendation_service),
) -> RecommendationResponse:
    t0 = time.perf_counter()
    logger.info("get_recommendations.start user_id=%s", user_id)
    try:
        response = service.get_recommendations(user_id)
        elapsed_ms = (time.perf_counter() - t0) * 1000
        logger.info(
            "get_recommendations.success user_id=%s elapsed_ms=%.1f count=%s",
            user_id,
            elapsed_ms,
        ...
        elapsed_ms = (time.perf_counter() - t0) * 1000
        logger.warning("get_recommendations.generation_error elapsed_ms=%.1f error=%s", elapsed_ms, exc)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
